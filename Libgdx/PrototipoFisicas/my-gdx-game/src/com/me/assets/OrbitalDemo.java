...
public class OrbitalDemo extends ApplicationAdapter {
	...
	private final float VIEWPORT_HEIGHT = 60f;
	private final float PLANET_WIDTH = 8f;
	private final float PLANET_CORE_WIDTH = PLANET_WIDTH;
	private final float ENEMY_WIDTH = 6f;
	...
	private Body enemyModel;
	private Vector2 enemyModelOrigin;
	...
	public void create() {
		...
		Gdx.input.setInputProcessor(new InputAdapter() {
			...
			public boolean scrolled (int amount) {
				joint.setLength(0f);
				//enemyModel.applyForce(new Vector2(1,1000), planetModelOrigin);
				enemyModel.setLinearVelocity(planetModelOrigin);
				return true;
			}
		});
	...
	private void createEnemy() {
		...
		enemyModel = world.createBody(bd);

		// 4. Create the body fixture automatically by using the loader.
		loader.attachFixture(enemyModel, "enemy", fd, ENEMY_WIDTH);
		enemyModelOrigin = loader.getOrigin("enemy", ENEMY_WIDTH).cpy();
		
		DistanceJointDef jointDef = new DistanceJointDef();        
		//jointDef.initialize(enemyModel, planetModel, enemyModelOrigin, planetModelOrigin);
		jointDef.initialize(enemyModel, planetModel, new Vector2(), new Vector2());
...
//		jointDef.bodyA = planetModel;
//		jointDef.bodyB = enemyModel;
//		jointDef.collideConnected = true;
//		jointDef.maxLength = 10;
	...
	private void createSprites() {
		...
		//enemy
		enemyTexture = new Texture(Gdx.files.internal("data/cr/Enemy.png"));
		enemyTexture.setFilter(TextureFilter.Linear, TextureFilter.Linear);
		enemySprite = new Sprite(enemyTexture);
		enemySprite.setSize(ENEMY_WIDTH, ENEMY_WIDTH*enemySprite.getHeight()/enemySprite.getWidth())		;
...
//		enemyModel.setAngularVelocity(5f);
//		
		enemyModel.setAngularDamping(0);
		enemyModel.setLinearDamping(0);
		planetModel.setAngularDamping(0);
		planetModel.setLinearDamping(0);
		//enemyModel.setAngularVelocity(15f);
		...
		Vector2 enemyPos = enemyModel.getPosition().sub(enemyModelOrigin);
		enemySprite.setPosition(enemyPos.x, enemyPos.y);
		enemySprite.setOrigin(enemyModelOrigin.x, enemyModelOrigin.y);
		enemySprite.setRotation(enemyModel.getAngle() * MathUtils.radiansToDegrees);
	...
	private Vector2 getTangentVector(Vector2 relativePosition, boolean clockwise){
	    Vector2 vector = new Vector2();
	    
	    if(clockwise){
	    	vector.x = -relativePosition.y;
	    	vector.y = relativePosition.x;
	    }
	...
	private void restart() {
		...
		enemyModel.setLinearVelocity(new Vector2(-DISTANCE,0));
